...
module.exports.startDevRoute = function(slug, port, callback) {
  var data, doctype, err, error, packageData, packagePath, perm, perms, token, _ref;
  if (token = getToken()) {
  ...
  perms = {};
  _ref = packageData['cozy-permissions'];
  for (doctype in _ref) {
    perm = _ref[doctype];
...
module.exports.moduleStatus = function(module, callback) {
  ...
  return helpers.clients[module].get('', function(err, res) {
    var _ref;
    if ((res == null) || ((_ref = !res.statusCode) === 200 || _ref === 401 || _ref === 403)) {
