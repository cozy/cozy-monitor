...
var allDocuments, async, configureCouchClient, couchClient, createDir, createFile, createFileStream, createViews, exportAlbums, exportContacts, exportDirs, exportFiles, exportOthers, exportPhotos, fetchLocale, fs, getAllElements, getContent, getDirs, getFiles, helpers, locale, log, request, tar, vcardParser, zlib;
...
createViews = function(callback) {
  var views;
  views = {
    views: {
      files: {
        map: "function (doc) { if (doc.docType && doc.docType.toLowerCase() === 'file') { emit(doc.path, doc) } }"
      },
      folders: {
        map: "function (doc) { if (doc.docType && doc.docType.toLowerCase() === 'folder') { emit(doc.path, doc) } }"
      }
    }
  };
  return couchClient.put('cozy/_design/cozy-monitor-export', views, callback);
};

getFiles = function(couchClient, callback) {
  return couchClient.get('cozy/_design/cozy-monitor-export/_view/files', function(err, res, body) {
    if (err) {
      return callback(err);
    } else if (res.statusCode !== 200) {
      return callback("status code " + res.statusCode + " for files");
    } else {
...
getDirs = function(couchClient, callback) {
  return couchClient.get('cozy/_design/cozy-monitor-export/_view/folders', function(err, res, body) {
    if (err) {
      return callback(err);
    } else if (res.statusCode !== 200) {
      return callback("status code " + res.statusCode + " for folders");
    } else {
      return callback(null, body);
    }
  });
...
getAllElements = function(couchClient, element, callback) {
  return couchClient.get("cozy/_design/" + element + "/_view/all", function(err, res, body) {
    if (err) {
      ...
      return callback("status code " + res.statusCode + " for " + element);
    } else {
      return callback(null, body);
    }
  });
...
allDocuments = function(couchClient, start, forEach, done) {
  var limit, u;
  limit = 1000;
  u = "cozy/_all_docs?include_docs=true&limit=" + limit;
  if (start != null) {
    u += "&skip=1&startkey=\"" + (encodeURIComponent(start)) + "\"";
  }
  return couchClient.get(u, function(err, res, body) {
    if (err != null) {
      return done(err);
    } else {
      return async.eachSeries(body.rows, function(row, cb) {
        return forEach(row.doc, cb);
      }, function(err) {
        if (err) {
          return done(err);
        } else if (body.rows.length === limit) {
          start = body.rows[limit - 1].id;
          return allDocuments(couchClient, start, forEach, done);
        } else {
          return done(null);
        }
      });
...
createFileStream = function(pack, name, size, stream, callback) {
  var chunks;
  if (+size > 500000000) {
    return stream.pipe(pack.entry({
      name: name,
      size: size,
      mode: 0x1a0,
      mtime: new Date,
...
createFile = function(pack, name, data, callback) {
  var entry;
  entry = pack.entry({
    name: name,
...
exportDirs = function(pack, next) {
  return getDirs(couchClient, function(err, dirs) {
    ...
    return async.eachSeries(dirs.rows, function(dir, cb) {
  ...
  return getFiles(couchClient, function(err, files) {
    ...
    return async.eachSeries(files.rows, function(file, cb) {
      ...
      return getContent(couchClient, binaryId, 'file', function(err, stream) {
        ...
        size = file != null ? (ref4 = file.value) != null ? ref4.size : void 0 : void 0;
        return createFileStream(pack, name, size, stream, cb);
      });
    }, function(err, value) {
      if (err) {
...
exportPhotos = function(pack, references, next) {
  return getAllElements(couchClient, 'photo', function(err, photos) {
    var dir, dirname;
    if (err === "status code 404 for photo") {
      return next(null);
    }
    if (err != null) {
      return next(err);
    ...
    return createDir(pack, dir, function() {
      return async.eachSeries(photos.rows, function(photo, cb) {
        ...
        return getContent(couchClient, binaryId, 'raw', function(err, stream) {
          ...
          return createFileStream(pack, dir + "/" + name, 0, stream, cb);
        });
      }, function(err, value) {
        if (err) {
...
exportAlbums = function(pack, references, next) {
  return getAllElements(couchClient, 'album', function(err, albums) {
    var albumsref;
    if (err === "status code 404 for album") {
      return next(null);
    }
    if (err != null) {
      return next(err);
    ...
    return async.eachSeries(albums.rows, function(album, cb) {
      ...
      return createDir(pack, 'albums', function(err) {
        ...
        return createFile(pack, 'albums/albums.json', albumsref, function(err) {
          var ref;
          if (err != null) {
            return next(err);
          }
          ref = references.join('\n');
          return createFile(pack, 'albums/references.json', ref, function(err) {
            if (err != null) {
              log.info('Error while exporting albums: ', err);
            } else {
  ...
  return getAllElements(couchClient, 'contact', function(err, contacts) {
    ...
    return createDir(pack, 'contacts', function() {
      return async.eachSeries(contacts.rows, function(contact, cb) {
        ...
        return createFile(pack, "contacts/" + filename, vcard, cb);
      }, function(err, value) {
        if (err) {
...
exportOthers = function(pack, next) {
  var doctypes, other, save, saveFile;
  doctypes = {};
  other = "Archive";
  saveFile = function(doc, doctype, callback) {
    var data, name;
    name = "files/" + other + "/" + doctype + "/" + doc._id;
    data = JSON.stringify(doc);
    return createFile(pack, name, data, callback);
  };
  save = function(doc, callback) {
    ...
    if (doctype === "binary" || doctype === "contact") {
      return callback();
    }
    if (doctype === "album" || doctype === "photo") {
      return callback();
    }
    if (doctypes[doctype]) {
      return saveFile(doc, doctype, callback);
    } else {
      doctypes[doctype] = true;
      return createDir(pack, "files/" + other + "/" + doctype, function(err) {
        if (err != null) {
          return callback(err);
        } else {
          return saveFile(doc, doctype, callback);
  ...
  return allDocuments(couchClient, null, save, function(err) {
    if (err != null) {
