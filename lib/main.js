...
var STACK, application, appsPath, async, axon, clients, cozyStack, db, exec, exp, fs, helpers, humanize, log, logError, monitoring, path, pkg, program, request, stackApplication, version,
...
program.command("export [tarball]").description("Export the files, photos, albums, and contacts to a tarball").action(function(filename) {
  if (filename == null) {
    filename = "cozy.tar.gz";
  }
  return exp.exportDoc(filename, function(err) {
    if (err != null) {
      return logError(err);
    } else {
